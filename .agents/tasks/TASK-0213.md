...
## Acceptance criteria
- After seeding transcript events, `/admin/postcall/extract` writes 2–3 structured postcall events.
- Tests cover:
  - schema validation failure path
  - idempotency behavior
  - tenant isolation

## Integration note (resolved)
- Initial integration failures (`transcript_not_found`) occurred when Flow A stored only `transcript_len`.
- Resolved by persisting transcript text on `flow_a.transcript_completed` payload (`transcript` + `transcript_len`).
- Verified with real call rid:
  - extractor returned `ok: true`
  - wrote `postcall.summary` and `postcall.lead` events.
